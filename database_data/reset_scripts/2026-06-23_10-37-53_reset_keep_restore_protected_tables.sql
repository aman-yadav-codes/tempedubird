...
-- Detach preserved rows from soon-to-be-cleared user/institution rows before copying them.
...
-- Copy protected data into transaction-local temporary tables.
...
-- Clear the database. Protected data is restored immediately after this.
TRUNCATE TABLE
...
-- Repair sequences for restored tables that kept explicit IDs.
DO $$
DECLARE
  table_name text;
  seq_name text;
...
BEGIN
  FOREACH table_name IN ARRAY ARRAY[
    'app_migrations','scope_types','permissions','roles','boards','categories','sections','subjects','card_categories','syllabi','syllabus_nodes','syllabus_inheritance_logs'
  ] LOOP
    SELECT pg_get_serial_sequence('public.' || quote_ident(table_name), 'id') INTO seq_name;
