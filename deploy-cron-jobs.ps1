# Deploy the standalone EduBird cron worker to the Ubuntu VM and run it with PM2.
...
$LOCAL_DIR = "Edubird\cron-jobs"
$REMOTE_ROOT = "/home/ubuntu/Edubird"
$REMOTE_DIR = "$REMOTE_ROOT/cron-jobs"
$PROCESS_NAME = "edubird-cron-jobs"
$TAR_NAME = "edubird-cron-jobs.tar.gz"

Write-Host "=============================================" -ForegroundColor Cyan
Write-Host "Starting EduBird cron worker deployment..." -ForegroundColor Cyan
Write-Host "=============================================" -ForegroundColor Cyan

if (-not (Test-Path $LOCAL_DIR)) {
    Write-Error "Local cron worker folder not found: $LOCAL_DIR"
    Exit 1
}

Write-Host "[1/6] Checking local worker syntax..." -ForegroundColor Yellow
Push-Location $LOCAL_DIR
npm run check
$checkStatus = $?
Pop-Location
if (-not $checkStatus) {
    Write-Error "Local worker syntax check failed."
    Exit 1
}

Write-Host "[2/6] Packaging cron worker source..." -ForegroundColor Yellow
...
if (-not $?) {
    Write-Error "Failed to package cron worker."
    Exit 1
}
...
Write-Host "[6/6] Starting cron worker via PM2..." -ForegroundColor Yellow
...
if (-not $?) {
    Write-Error "Failed to start cron worker under PM2."
    Remove-Item $TAR_NAME -Force
    Exit 1
}
