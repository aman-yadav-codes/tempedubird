...
function mapPublicCourseRow(row: Record<string, unknown>) {
  ...
  const institution = {
    ...
    phone: typeof row.institution_phone === "string" ? row.institution_phone : null,
    email: typeof row.institution_email === "string" ? row.institution_email : null,
  ...
  const result = await db.query(
    `
      ...
      SELECT
        ...
        inst.email AS institution_email,
