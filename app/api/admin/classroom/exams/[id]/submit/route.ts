  ...
  const result = await db.query<{ version: number }>(
    `
      ...
      INNER JOIN practice_exams exam
       ...
       AND (exam.exam_date + exam.exam_time) <= (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Kolkata')
...
export async function POST(req: Request, context: Context) {
  try {
    ...
    const exam = studentId
      && enrollment ? await getAccessibleExam(examId, studentId, enrollment.id)
      : null;
    if (!studentId || !exam) {
      throw new Error("Practice Exam not found");
    }
    const examVersion = Number(exam.version ?? 1);
    ...
    if (questions.rows.length === 0) throw new Error("This exam has no questions");
    ...
    try {
      ...
      await client.query(`SELECT pg_advisory_xact_lock($1, $2)`, [examId, studentId]);
      const existingAttempt = await client.query(
        `SELECT 1 FROM student_practice_exam_attempts
         WHERE practice_exam_id = $1 AND student_id = $2 AND enrollment_id = $3 AND status = 'completed'
           AND COALESCE(is_deleted, FALSE) = FALSE
         LIMIT 1`,
        [examId, studentId, enrollment.id]
      );
      if (existingAttempt.rows[0]) {
        throw new Error("This exam has already been completed and cannot be submitted again");
      }
      const attemptResult = await client.query<{ id: number }>(
        `
          ...
          RETURNING id
        `,
        [examId, studentId, enrollment.id, examVersion]
      );
      ...
      for (const question of questions.rows) {
        ...
        const answerImageUrl = String(rawAnswer?.answer_image_url ?? "").trim();
        ...
        } else if (!answerText && !answerImageUrl) {
          unanswered += 1;
        } else {
          isCorrect = null;
          unanswered += 1;
        ...
        await client.query(
          `
            INSERT INTO student_practice_exam_answers
              (attempt_id, question_id, selected_option_id, answer_text, answer_image_url, is_correct, marks_awarded)
