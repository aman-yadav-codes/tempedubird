    ...
    const result = await db.query(
      `
        ...
        SELECT
          eligible.*,
          CASE
            WHEN (eligible.exam_date + eligible.exam_time) > (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Kolkata') THEN 0
          ...
          sa.submitted_at,
          CASE WHEN eligible.instant_result OR eligible.result_date <= (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Kolkata')::date THEN sa.obtained_marks::float8 END AS obtained_marks,
          CASE WHEN eligible.instant_result OR eligible.result_date <= (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Kolkata')::date THEN sa.percentage::float8 END AS percentage,
          CASE WHEN eligible.instant_result OR eligible.result_date <= (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Kolkata')::date THEN sa.correct_answers END AS correct_answers,
          CASE WHEN eligible.instant_result OR eligible.result_date <= (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Kolkata')::date THEN sa.wrong_answers END AS wrong_answers,
          CASE WHEN eligible.instant_result OR eligible.result_date <= (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Kolkata')::date THEN sa.unanswered END AS unanswered,
          (eligible.instant_result OR eligible.result_date <= (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Kolkata')::date) AS result_available,
          ((eligible.exam_date + eligible.exam_time) <= (CURRENT_TIMESTAMP AT TIME ZONE 'Asia/Kolkata')) AS is_released,
        ...
        GROUP BY
          ...
          eligible.exam_place,
          eligible.exam_mode,
          eligible.result_date,
          eligible.instant_result,
          eligible.version,
          eligible.status,
