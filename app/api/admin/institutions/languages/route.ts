...
import { listLanguages, createLanguage } from "@/lib/queries/institutions";
...
export async function GET(req: Request) {
    try {
        ...
        const { data, total } = await listLanguages(db, { search, limit, offset });
...
export async function POST(req: Request) {
    try {
        ...
        const item = await createLanguage(db, parsed);
...
export async function PATCH(req: Request) {
    try {
        ...
        if (typeof isActive === "boolean") {
            await db.query(
                `UPDATE languages SET is_active = $1, updated_at = NOW() WHERE id = ANY($2::int[])`,
                [isActive, numericIds]
            );
        }
        if (softDelete === true) {
            await db.query(
                `UPDATE languages SET is_deleted = TRUE, updated_at = NOW() WHERE id = ANY($1::int[])`,
                [numericIds]
            );
        }
