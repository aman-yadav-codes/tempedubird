...
import { listInstitutionTypes, createInstitutionType } from "@/lib/queries/institutions";
...
export async function GET(req: Request) {
    try {
        ...
        const { data, total } = await listInstitutionTypes(db, { search, limit, offset });
...
export async function POST(req: Request) {
    try {
        ...
        const item = await createInstitutionType(db, { name, slug });
...
export async function PATCH(req: Request) {
    try {
        ...
        if (typeof isActive === "boolean") {
            await db.query(
                `UPDATE institution_types SET is_active = $1, updated_at = NOW() WHERE id = ANY($2::int[])`,
                [isActive, numericIds]
            );
        }
        if (softDelete === true) {
            await db.query(
                `UPDATE institution_types SET is_deleted = TRUE, updated_at = NOW() WHERE id = ANY($1::int[])`,
                [numericIds]
            );
        }
