...
export async function GET(req: NextRequest) {
    try {
        ...
        const totalResult = await db.query(
            `SELECT COUNT(*) as count FROM skills WHERE is_deleted = false`
        );
        const total = totalResult.rows[0]?.count || 0;

        // Get active count
        const activeResult = await db.query(
            `SELECT COUNT(*) as count FROM skills WHERE is_deleted = false AND is_active = true`
        );
        const active = activeResult.rows[0]?.count || 0;

        // Get disabled count
        const disabledResult = await db.query(
            `SELECT COUNT(*) as count FROM skills WHERE is_deleted = false AND is_active = false`
        );
        const disabled = disabledResult.rows[0]?.count || 0;

        // Get deleted count
        const deletedResult = await db.query(
            `SELECT COUNT(*) as count FROM skills WHERE is_deleted = true`
        );
