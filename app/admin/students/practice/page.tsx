...
export default function StudentPracticePage() {
  ...
  const [rows, setRows] = useState<PracticeAttemptRow[]>([]);
  ...
  const [detail, setDetail] = useState<PracticeAttemptDetail | null>(null);
  ...
  const fetchAttempts = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/students/practice?${params}`, {
        headers: authHeader,
      });
      const json = await readJson(res);
      if (!res.ok) throw new Error(json.error ?? "Failed to load practice attempts");
      setRows(json.data ?? []);
      setPageCount(json.pageCount ?? 0);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to load practice attempts");
    } finally {
      setLoading(false);
    }
  }, [
  ...
  const openAttempt = useCallback(
    async (row: PracticeAttemptRow) => {
      ...
      try {
        ...
        const json = await readJson(res);
        if (!res.ok) throw new Error(json.error ?? "Failed to load practice attempt");
        setDetail(json.data ?? null);
      } catch (error) {
        toast.error(error instanceof Error ? error.message : "Failed to load practice attempt");
      } finally {
  ...
  const columns = useMemo<ColumnDef<PracticeAttemptRow>[]>(
    () => [
      {
        accessorKey: "title",
        ...
        cell: ({ row }) => (
          <div className="min-w-72">
            ...
            <p className="text-xs text-muted-foreground">
              v{row.original.exam_version} · Attempt #{row.original.attempt_no}
            </p>
          </div>
        ),
      },
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-emerald-600 dark:text-emerald-400">
            {row.original.correct_answers ?? 0}
          </span>
        ),
      },
      {
        ...
        header: "Wrong",
        cell: ({ row }) => (
          <span className="text-destructive">{row.original.wrong_answers ?? 0}</span>
        ),
      },
      {
  ...
  if (!isReady) {
    return <div className="text-muted-foreground">Loading practice attempts...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <div>
          ...
          <p className="text-muted-foreground">
            Review student practice exam attempts, marks, and answer results.
          </p>
        </div>
      </div>
      ...
      <DataTable
        ...
        emptyText={
          institutionId
            ? "No student practice attempts found."
            : "Select an institution from the sidebar to view practice attempts."
        }
        manualPagination
        pageCount={pageCount}
        ...
        toolbarLeft={
          <Input
            ...
            placeholder="Search practice exams or students..."
        ...
        toolbarRight={
          ...
          >
            ...
            <span className="sr-only">Refresh practice attempts</span>
          </Button>
        }
      />
      ...
      >
        <SheetContent
          ...
          defaultSize={760}
          resizeStorageKey="student-practice-attempt-review-sheet"
        >
          <SheetHeader className="border-b px-6 py-5">
            <SheetTitle>{detail?.attempt.title ?? "Practice Attempt"}</SheetTitle>
            <SheetDescription>
              {detail
                ? `${detail.attempt.student_name} · Attempt #${detail.attempt.attempt_no} · Submitted ${formatDateTime(detail.attempt.submitted_at)}`
                : "Loading student attempt..."}
            </SheetDescription>
            {detail ? (
