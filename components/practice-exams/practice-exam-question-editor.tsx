...
type EditorQuestion = PracticeExamQuestion & {
  client_id: string;
  open: boolean;
};
...
function toUploadFiles(
  files: PracticeExamQuestion["files"]
...
export function PracticeExamQuestionEditor({
  open,
  onOpenChange,
  accessToken,
  template,
  ...
  function changeQuestionType(id: string, questionType: PracticeExamQuestionType) {
  ...
  async function generateWithAi() {
    ...
    try {
      const res = await fetch(
        `/api/admin/master-data/practice-exams/${template.id}/questions/generate`,
        {
          method: "POST",
          headers: {
      ...
      const generated = (json.data ?? []) as PracticeExamQuestion[];
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/master-data/practice-exams/${template.id}/questions`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      toast.success("Practice Exam questions saved");
      onOpenChange(false);
      onSaved();
    } catch (error) {
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value) =>
                            changeQuestionType(
                              question.client_id,
                              value as PracticeExamQuestionType
                            )
                          }
                        >
