...
interface MultiSelectOption {
    ...
    description?: string;
    /** Optional small badge displayed at the end of an option row. */
    badge?: string;
...
interface MultiSelectProps
    ...
    /**
     * If false, selectedOptions are used only to resolve selected labels/badges
     * and are not shown or counted in the dropdown list.
     * Optional, defaults to true.
     */
    includeSelectedOptionsInDropdown?: boolean;

    async?: boolean;

    /**
     * Async fetcher for paginated options
     */
    fetcher?: (
        search: string,
        page: number
    ...
    /** Shows a loading spinner and prevents interaction while options are changing. */
    loading?: boolean;
    ...
    (
        {
            options = [],
            selectedOptions = [],
            includeSelectedOptionsInDropdown = true,
            onValueChange,
            variant,
            defaultValue = [],
            value,
        ...
        const [popoverSide, setPopoverSide] =
            React.useState<React.ComponentProps<typeof PopoverContent>["side"]>("bottom");
        const [popoverAvailableHeight, setPopoverAvailableHeight] =
            React.useState(320);
        const [isAnimating, setIsAnimating] = React.useState(false);
        const [searchValue, setSearchValue] = React.useState("");
        const [asyncOptions, setAsyncOptions] =
            React.useState<MultiSelectOption[]>([]);
        ...
        const getAllOptions = React.useCallback((): MultiSelectOption[] => {
            ...
            const source: MultiSelectOption[] = async
                ? [
                    ...asyncOptions,
                    ...flatOptions,
                    ...selectedOptions,
            ...
            const allOptions: MultiSelectOption[] = source;
            ...
            if (process.env.NODE_ENV === "development" && duplicates.length > 0 && !deduplicateOptions) {
                console.warn(
        ...
        }, [options, selectedOptions, asyncOptions, async, deduplicateOptions, isGroupedOptions]);
        ...
        React.useEffect(() => {
            if (!async || !fetcher || !isPopoverOpen) {
                return;
            }
        ...
        const displayedOptions = React.useMemo<MultiSelectOption[] | MultiSelectGroup[]>(() => {
            const flatOptions = isGroupedOptions(options)
                ? options.flatMap((group) => group.options)
                : options;

            if (!async && !includeSelectedOptionsInDropdown) {
                return options;
            }
            ...
            const merged = [
                ...(async ? asyncOptions : []),
                ...flatOptions,
                ...(includeSelectedOptionsInDropdown ? selectedOptions : []),
            ];
            ...
            options,
            selectedOptions,
            includeSelectedOptionsInDropdown,
            asyncOptions,
            isGroupedOptions,
            selectedValues,
        ]);
        const filteredOptions = React.useMemo(() => {
            if (!searchable || !searchValue) return displayedOptions;
            if (displayedOptions.length === 0) return [];
            if (isGroupedOptions(displayedOptions)) {
        ...
        }, [displayedOptions, searchValue, searchable, isGroupedOptions]);

        const flattenedFilteredOptions = React.useMemo<MultiSelectOption[]>(() => {
            return isGroupedOptions(filteredOptions)
                ? filteredOptions.flatMap((group) => group.options)
                : filteredOptions;
        }, [filteredOptions, isGroupedOptions]);

        const filteredSelectableOptions = React.useMemo(
            () => flattenedFilteredOptions.filter((option) => !option.disabled),
            [flattenedFilteredOptions]
        );

        const allFilteredOptionsSelected =
            filteredSelectableOptions.length > 0 &&
            filteredSelectableOptions.every((option) =>
        ...
        const updatePopoverLayout = React.useCallback(() => {
            ...
            const preferredSide = spaceAbove > spaceBelow && spaceBelow < 280 ? "top" : "bottom";
            const availableHeight = preferredSide === "top" ? spaceAbove : spaceBelow;

            setPopoverSide(preferredSide);
            setPopoverAvailableHeight(Math.max(220, Math.min(520, availableHeight)));
        }, []);
        ...
        const toggleAll = () => {
            ...
            if (allFilteredOptionsSelected) {
                ...
                const allValues = filteredSelectableOptions.map((option) => option.value);
                ...
                <Popover
                    ...
                    <PopoverTrigger asChild>
                        <Button
                            ...
                            className={cn(
                                "flex p-1 rounded-md border border-input bg-background min-h-10 h-auto items-center justify-between shadow-sm hover:bg-background [&_svg]:pointer-events-auto",
                    ...
                    <PopoverContent
                        ...
                        className={cn(
                            "w-(--radix-popper-anchor-width) max-w-[calc(100vw-2rem)] p-0 overflow-hidden",
                            getPopoverAnimationClass(),
                            screenSize === "mobile" && "w-[calc(100vw-2rem)] max-w-70",
                            screenSize === "tablet" && "w-[min(70vw,28rem)]",
                            screenSize === "desktop" && "min-w-0",
                            popoverClassName
                        )}
                        style={{
                            ...
                            maxHeight: popoverAvailableHeight,
                        ...
                        <Command>
                            ...
                            <CommandList
                                ...
                                style={{
                                    maxHeight: Math.max(160, popoverAvailableHeight - (searchable ? 64 : 20)),
                                }}>
                                <CommandEmpty>
                                ...
                                {!hideSelectAll && !searchValue && (
                                    <CommandGroup>
                                        <CommandItem
                                            ...
                                            aria-label={`Select all ${filteredSelectableOptions.length
                                                } options`}
                                            disabled={filteredSelectableOptions.length === 0}
                                            ...
                                            <span>
                                                (Select All - {filteredSelectableOptions.length} options)
                                            </span>
                                        </CommandItem>
                                    </CommandGroup>
                                )}
                                                    ...
                                                    <CommandItem
                                                        ...
                                                        <div className="min-w-0 flex-1">
                                                            <div className="flex min-w-0 items-center gap-2">
                                                                ...
                                                                {option.badge && (
                                                                    <span className="ml-auto shrink-0 rounded-full border border-border/70 bg-muted/70 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                                                                        {option.badge}
                                                                    </span>
                                                                )}
                                                            </div>
                                                            {option.description && (
                                                                <span className="mt-0.5 block min-w-0 truncate pl-0 text-[11px] leading-tight text-muted-foreground">
                                                                    {option.description}
                                                                </span>
                                                            )}
                                                        </div>
                                            ...
                                            return (
                                                <CommandItem
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        ...
                                                        {option.description && (
                                                            <span className="mt-0.5 block min-w-0 truncate text-[11px] leading-tight text-muted-foreground">
                                                                {option.description}
                                                            </span>
                                                        )}
                                                    </div>
