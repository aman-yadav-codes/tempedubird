...
import { useBlockViewer } from "@/components/editor/block-viewer-provider";
...
export function Editor({
  ...
  const [initialSerializedState] = useState(() => editorSerializedState);
  ...
  const AppExtension = useMemo(
    ...
    [initialEditorState, initialSerializedState, maxLength],
  );

  return (
    <div className="flex h-full min-h-0 w-full min-w-0 max-w-full flex-col overflow-hidden rounded-none border-0 bg-background shadow-none">
      <LexicalExtensionComposer extension={AppExtension} contentEditable={null}>
        <TooltipProvider>
          {alwaysEditable && <EnsureEditablePlugin />}
          <div className="relative flex h-full min-h-0 min-w-0 flex-col overflow-hidden">
            <ToolbarPlugin>
              {({ blockType }) => (
                <div className="sticky top-0 z-10 flex w-full min-w-0 max-w-full shrink-0 items-center gap-2 overflow-x-auto border-b bg-background p-1 [&>*]:shrink-0">
            ...
            <div className="relative min-h-0 min-w-0 flex-1 overflow-visible">
              <div className="h-full min-h-0 min-w-0">
                <div className="h-full min-h-0 min-w-0" ref={onRef}>
                  <ContentEditable
            ...
            <ActionsPlugin>
              <div className="clear-both flex w-full min-w-0 max-w-full shrink-0 flex-col items-center justify-between gap-3 border-t p-2 md:flex-row">
                <div className="flex flex-col sm:flex-row items-center gap-3 text-xs text-zinc-500 dark:text-zinc-400 w-full md:w-auto justify-center md:justify-start">
