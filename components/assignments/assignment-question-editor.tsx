...
type EditorQuestion = AssignmentTemplateQuestion & {
  client_id: string;
  open: boolean;
};
...
function toUploadFiles(
  files: AssignmentTemplateQuestion["files"]
...
export function AssignmentQuestionEditor({
  ...
  function changeQuestionType(id: string, questionType: AssignmentQuestionType) {
  ...
  async function generateWithAi() {
    ...
    try {
      const res = await fetch(
        `/api/admin/master-data/assignments/${template.id}/questions/generate`,
        {
          method: "POST",
          headers: {
      ...
      const generated = (json.data ?? []) as AssignmentTemplateQuestion[];
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/master-data/assignments/${template.id}/questions`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      toast.success("Assignment questions saved");
      onOpenChange(false);
      onSaved();
    } catch (error) {
