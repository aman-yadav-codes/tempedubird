...
type EditorQuestion = NoteTemplateQuestion & {
  client_id: string;
  open: boolean;
};
...
function toUploadFiles(
  files: NoteTemplateQuestion["files"] | undefined
...
export function NoteQuestionEditor({
  open,
  onOpenChange,
  accessToken,
  template,
      ...
      if ((template.questions?.length ?? 0) > 0) {
        setQuestions(
          (template.questions ?? []).map((question, index) => ({
            ...
            answer_files: toUploadFiles(question.answer_files),
            open: index === 0,
          }))
        );
      } else {
  ...
  function changeQuestionType(id: string, questionType: NoteQuestionType) {
  ...
  function validate() {
    ...
    if (questions.length === 0) nextErrors.push("Add at least one Q&A note entry.");

    questions.forEach((question, index) => {
      let questionInvalid = false;
      if (!question.question_text.trim()) {
        nextErrors.push(`Entry ${index + 1}: question text is required.`);
        questionInvalid = true;
      }
      if (question.files.length > 5) {
        nextErrors.push(`Entry ${index + 1}: only 5 question images are allowed.`);
        questionInvalid = true;
      }
      if ((question.answer_files?.length ?? 0) > 5) {
        nextErrors.push(`Entry ${index + 1}: only 5 answer files are allowed.`);
        questionInvalid = true;
      }
      if (question.question_type === "objective") {
        if (question.options.length < 2) {
          nextErrors.push(`Entry ${index + 1}: add at least two options.`);
          questionInvalid = true;
        }
        if (question.options.some((option) => !option.text.trim())) {
          nextErrors.push(`Entry ${index + 1}: all options are required.`);
          questionInvalid = true;
        }
        if (question.options.filter((option) => option.is_correct).length !== 1) {
          nextErrors.push(
            `Entry ${index + 1}: select exactly one correct answer.`
          );
          questionInvalid = true;
        }
      }
      if (
        ...
        nextErrors.push(`Entry ${index + 1}: select True or False.`);
        questionInvalid = true;
      }
  ...
  async function generateWithAi() {
    ...
    try {
      const res = await fetch(
        `/api/admin/master-data/notes/${template.id}/questions/generate`,
        {
          method: "POST",
          headers: {
      ...
      if (!res.ok) {
        if (
          ...
          toast.error(json.error ?? "Configure API key first");
          window.setTimeout(() => {
            window.location.href = json.redirectTo ?? "/admin/ai-settings";
          }, 450);
          return;
        }
        throw new Error(json.error ?? "Failed to generate Q&A entries");
      }
      const generated = (json.data ?? []) as NoteTemplateQuestion[];
      if (generated.length === 0) throw new Error("AI did not return Q&A entries");
      setQuestions(
      ...
      toast.success("AI Q&A notes generated with answers");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to generate Q&A entries");
    } finally {
    ...
    try {
      const payload = {
        questions: questions.map((question, index) => ({
          ...
          answer_files: (question.answer_files || []).map((file) => ({
            url: file.url,
            name: file.name,
          })),
        })),
      ...
      const res = await fetch(
        `/api/admin/master-data/notes/${template.id}/questions`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      const json = await readJson(res);
      if (!res.ok) throw new Error(json.error ?? "Failed to save Q&A notes");
      toast.success("Q&A note entries saved successfully");
      onOpenChange(false);
      onSaved();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Failed to save Q&A notes");
    } finally {
      setSaving(false);
    }
  }
          ...
          <div className="flex flex-col gap-3 pr-8 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0">
              <DialogTitle className="flex items-center gap-2">
                ...
                <span className="truncate">Q&A Entries for {template.title}</span>
              </DialogTitle>
              <DialogDescription className="sr-only">
                Questions and answers for {template.title}
              </DialogDescription>
            </div>
                    ...
                    <CollapsibleTrigger asChild>
                      ...
                      >
                        ...
                        <span className="min-w-0 flex-1 truncate font-medium">
                          {question.question_text || `Q&A Entry ${questionIndex + 1}`}
                        </span>
                        <ChevronDown
                          className={cn(
                  ...
                  <CollapsibleContent>
                    <div className="space-y-5 p-5">
                      ...
                      <div className="space-y-2">
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="subjective">Subjective / Theory</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                      ...
                      <div className="space-y-2 rounded-md border border-emerald-500/30 bg-emerald-500/5 p-4">
                        <Label className="flex items-center gap-2 font-semibold text-emerald-800 dark:text-emerald-300">
                        ...
                        <Textarea
                          ...
                          placeholder="Provide the complete answer, detailed step-by-step solution, or explanation..."
                      ...
                      {/* Question Images */}
                      <div className="space-y-2">
                        <Label>Question Diagram / Images (Optional, max 5)</Label>
                        <DocumentFileUpload
                      ...
                      {/* Answer Attachments */}
                      <div className="space-y-2">
                        <Label>Answer / Solution Diagrams & Attachments (Optional, max 5)</Label>
                        <DocumentFileUpload
                          accessToken={accessToken}
                          files={toUploadFiles(question.answer_files)}
                          onFilesChange={(files) =>
                            updateQuestion(question.client_id, (current) => ({
                              ...current,
                              answer_files: files,
